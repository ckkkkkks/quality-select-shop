...
#define ILLEGAL_STATE_EXN  "java/lang/IllegalStateException"
#define IO_EXN  "java/io/IOException"
...
#ifdef OPENSSL_3
#define RSA_KEY EVP_PKEY
#else
#define RSA_KEY RSA
#endif

static RSA_KEY *g_rsa_512 = 0;
static RSA_KEY *g_rsa_1024 = 0;
static RSA_KEY *g_rsa_4096 = 0;
 ...
 */
static int
ssl_verify_callback_none(int ok, X509_STORE_CTX *ctx)
{
  return 0;
}  
...
#ifdef OPENSSL_3
static RSA_KEY *
rsa_new(JNIEnv *env, int keylen)
{
    EVP_PKEY *key = EVP_RSA_gen(keylen);

    if (! key) {
      resin_printf_exception(env, "com/caucho/config/ConfigException", "OpenSSL can't generate a %d bit RSA key.  On Unix, check the machine's /dev/random and /dev/urandom configuration.", keylen);
...
rsa_new(JNIEnv *env, int keylen)
{
    ...
    if (! bne || BN_set_word(bne, RSA_F4) != 1) {
      resin_printf_exception(env, "com/caucho/config/ConfigException", "OpenSSL can't generate a %d bit RSA key BNE.  On Unix, check the machine's /dev/random and /dev/urandom configuration.", keylen);

      return 0;
    }

    if (RSA_generate_key_ex(key, keylen, bne, NULL) != 1) {
      resin_printf_exception(env, "com/caucho/config/ConfigException", "OpenSSL can't generate a %d bit RSA key.  On Unix, check the machine's /dev/random and /dev/urandom configuration.", keylen);
                   ...
                   const char *password)
{
  ...
  */
...
#ifdef OPENSSL_3
  if (config->crypto_device) {
    resin_printf_exception(env, "com/caucho/config/ConfigException", "crypto-device '%s' is unavailable in openssl 3");
    return 0;
  }
#else
#if defined(SSL_ENGINE)
  ...
  if (! (config->alg_flags & ALG_TLS1_3)) {
  ...
  if (certificate_chain_file &&
      SSL_CTX_use_certificate_chain_file(ctx, certificate_chain_file) != 1) {
    ERR_print_errors_fp(stderr); 
  ...
  /* SSL_CTX_set_options(ctx, SSL_OP_ALL|options); */
  ...
  if (config->legacy_renegotiation) {
    SSL_CTX_set_options(ctx, SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION);
  }
#endif

  if (! g_rsa_512) {
    g_rsa_512 = rsa_new(env, 512);
...
ssl_fill_ciphers(connection_t *conn)
{
  ...
  if (cipher) {
    ...
    conn->ssl_protocol = (void *) SSL_get_version(ssl);
  }
  else {
    conn->ssl_protocol = 0;
    conn->ssl_cipher = 0;
...
ssl_open(connection_t *conn, int fd)
{
  ...
  if (! ss || ss->fd <= 0 || ! ss->ssl_config) {
    resin_printf_exception(conn->jni_env, ILLEGAL_STATE_EXN,
  ...
  if (fd <= 0) {
    resin_printf_exception(conn->jni_env, ILLEGAL_STATE_EXN,
                           "ssl_open called with invalid fd");
  ...
  if (ssl) {
    resin_printf_exception(conn->jni_env, "java/io/IOException",
			   "OpenSSL open with existing socket");
...
init_ssl_engine(JNIEnv *env, ssl_config_t *config)
{
  ...
  resin_printf_exception(env, "com/caucho/config/ConfigException",
			 "<openssl> cannot initialize crypto-device for openssl3");
}
#else
...
Java_com_caucho_vfs_OpenSSLFactory_addConfigHost(JNIEnv *env,
                                                 jobject obj,
                                                 jlong p_config,
                                                 jstring id,
                                                 jstring jCertificateFile,
                                                 ...
                                                 jstring jPassword)
{
  ...
  host = cse_malloc(sizeof(ssl_host_t));
  memset(host, 0, sizeof(ssl_host_t));

  host->next = config->hosts;
  host->id = strdup_utf(env, id);
  host->certificate_file = strdup_utf(env, jCertificateFile);
  host->certificate_chain_file = strdup_utf(env, jCertificateChainFile);
  host->key_file = strdup_utf(env, jKeyFile);
  host->password = strdup_utf(env, jPassword);
					     ...
					     jint jVerifyDepth)
{
  ...
  if (jVerifyClient) {
    ...
    if (! string) {
      resin_printf_exception(env, ILLEGAL_STATE_EXN,
					      ...
					      jlong p_config)
{
  ...
  if (! ss) {
    resin_throw_exception(env, ILLEGAL_STATE_EXN,
...
ssl_servername_cb(SSL *s, int *ad, void *arg)
{
  ...
  ssl_sock_host_t *host;

  if (! servername || ! conn) {
    return SSL_TLSEXT_ERR_NOACK;
  }

  for (host = conn->ssl_hosts; host; host = host->next) {
    if (! strcmp(host->id, servername)) {
      SSL_set_SSL_CTX(s, host->ssl_context);
      conn->ssl_sni_context = host->ssl_context;

      return SSL_TLSEXT_ERR_OK;
    }
  }
  
  SSL_set_SSL_CTX(s, conn->ssl_context);
					...
					jlong p_config)
{
  ...
  */
  ...
  /*
  else if (conn->ss->context && ! config->hosts) {
    conn->ssl_context = conn->ss->context;
  }
  */
  else {
    conn->ssl_context = ssl_create_context(env,
                                           config,
                                           config->certificate_file,
                                           config->certificate_chain_file,
                                           config->key_file,
    ...
    if (config->hosts) {
      ...
      SSL_CTX_set_tlsext_servername_callback(conn->ssl_context,
                                             ssl_servername_cb);
      SSL_CTX_set_tlsext_servername_arg(conn->ssl_context, conn);

      for (host = config->hosts; host; host = host->next) {
        ssl_sock_host_t *ssl_host;

        ssl_host = (ssl_sock_host_t *) cse_malloc(sizeof(ssl_sock_host_t));

        ssl_host->next = conn->ssl_hosts;
        conn->ssl_hosts = ssl_host;

        ssl_host->id = host->id;
        ssl_host->ssl_context = ssl_create_context(env,
                                                   config,
                                                   host->certificate_file,
                                                   host->certificate_chain_file,
                                                   host->key_file,
                                                   host->password);
      
        SSL_CTX_set_tlsext_servername_callback(ssl_host->ssl_context,
                                               ssl_servername_cb);
        SSL_CTX_set_tlsext_servername_arg(ssl_host->ssl_context, 0);
      }
    }
